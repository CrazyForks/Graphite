	...
	pub fn collate_properties(context: &mut NodePropertiesContext) -> Vec<LayoutGroup> {
		...
		match *layers.as_slice() {
			...
			[layer] => {
				...
				// Mark each node in the layer's chain (but not the layer node itself, which is first) as draggable so its
				// section can be reordered within the chain from the Properties panel.
